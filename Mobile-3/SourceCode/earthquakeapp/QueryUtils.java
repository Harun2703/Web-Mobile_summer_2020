package com.example.harun.earthquakeapp;
...
public class QueryUtils {
    ...
    public static List<Earthquake> fetchEarthquakeData2(String requestUrl) {
        ...
        try {
            ...
            JSONObject object_json = new JSONObject(jsonResponse);
            // Fetching 'features' array.
            JSONArray array_json = (JSONArray) object_json.get("features");

            // Iterating the 'features' list
            for(int i =0;i < array_json.length(); i++){
                // Getting json Object 'properties'
                JSONObject json = array_json.getJSONObject(i).getJSONObject("properties");
                System.out.println(json);
                // Passing Data to Constructor
                Earthquake earthquake_constructor = new Earthquake((double)json.get("mag"),
                        (String) json.get("place"), (long)json.get("time"), (String) json.get("url"));
                // Adding each 'EarthQuake' Object to List created.
                earthquakes.add(earthquake_constructor);
            }
        } catch (Exception e) {
